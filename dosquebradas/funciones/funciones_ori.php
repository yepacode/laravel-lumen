...
{
    $sql = "select * from tbl_recaudo where referencia='$referencia' and estado='2'";
    $query = pg_query($sql);
...
function getArchivoConsolidado($entidad, $fecha)
{
    $fecha_inicio = fechaConsolidado($fecha);
    pg_close();
    include("conexion_pse.php");
    $sql = "SELECT * FROM tbl_transacciones
            WHERE   estado = 'OK' AND
                    cod_empresa = '$entidad' and
                    (   fecha_fin BETWEEN CAST('$fecha_inicio 17:00:00' AS TIMESTAMP) - CAST('1 days' AS INTERVAL) and CAST('$fecha 14:00:00' AS TIMESTAMP) and ciclo < 4
                        or
...
function getSumaConsolidado($entidad, $fecha)
{
    $fecha_inicio = fechaConsolidado($fecha);
    pg_close();
    include("conexion_pse.php");
    $sql = "SELECT  SUM(valor) FROM tbl_transacciones
            WHERE   estado = 'OK' AND
                    cod_empresa = '$entidad' and
                    (
                    fecha_fin BETWEEN CAST('$fecha_inicio 17:00:00' AS TIMESTAMP) - CAST('1 days' AS INTERVAL) and CAST('$fecha 14:00:00' AS TIMESTAMP) and ciclo < 4
...
function update_recaudo($referencia, $identificacion, $tipo_id, $nombre)
{
    ...
    if ($id_recibo) {
        $sql = "UPDATE tbl_recaudo  SET identificacion = '$identificacion', tipo_id = '$tipo_id', nombres = '$nombres' WHERE  id_recibo = '$id_recibo';";
